...
# Defining data
detention_data <- read_csv("file.csv")
...
## -------------------------- DESCRIPTION --------------------------------------
  ...
  div(
    ...
    p(
      ...
      " and represents immigration detentions from September 2023 through early July 2025. You can find more information on the data set, along with the ",
      a("codebook here", href = "https://deportationdata.org/docs/ice.html", target = "_blank"),
      "."
    ),
    p("Use this app to analyze patterns and trends within the detention dataset. Note that while this dataset is extensive, it may not capture all detentions or reflect ongoing updates. Data inaccuracies or reporting delays may affect some results. Please interpret findings accordingly.")
  ),
               ...
               div(class = "filter-subpanel",
                   ...
                   conditionalPanel(
                     ...
                     dateRangeInput("bookin_date_range", "Book-In Date Range:",
                                    start = "2023-01-01",
                                    end = max(detention_data$stay_book_in_date_time, na.rm = TRUE),
                                    min = min(detention_data$stay_book_in_date_time, na.rm = TRUE),
                                    max = max(detention_data$stay_book_in_date_time, na.rm = TRUE),
                                    format = "yyyy-mm-dd"
                     )
                   ),
...
## ------------------------------ HELPERS --------------------------------------
  ...
  apply_filters <- function(data) {
    if (input$filter_by_date && !is.null(input$bookin_date_range)) {
      data <- data %>% filter(
        stay_book_in_date_time >= input$bookin_date_range[1],
        stay_book_in_date_time <= input$bookin_date_range[2]
      )
    }
    if (input$non_active) {
